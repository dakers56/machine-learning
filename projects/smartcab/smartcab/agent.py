...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.epsilon = self.ep_func(a=.001, func_type='cos')
        self.trial += 1
        return None

    def ep_func(self, a=1, func_type='cos', exp=2):
        if func_type is 'cos':
            return math.fabs(math.cos(a * self.trial))
        if func_type is 'hyperbola':
            return 1. / (a * self.trial ** exp)
        if func_type is 'linear':
            return self.epsilon - a
        if func_type is 'exp':
            return math.exp(-a * self.trial)
        raise RuntimeError('Func %s not valid type of decay function')
    ...
    def createQ(self, state):
        ...
        if state not in self.Q:
            ...
            print("State-action-dictionary: %s" % {action: 0.0 for action in self.valid_actions})
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning=True, alpha=.99)
